...
public final class AdventuinParty {
    ...
    public static Map<HatType, Double> getDiffOfAvgHeightDiffsToPredecessorByHatType (List<Adventuin> adventuins)
    {
          ...
          groupByHatType(adventuins).forEach((key, value) -> {
              ...
              Double resultat = 0.0;
              Double mult = -1.0;
              if (Math.abs(negativeQuantity-0.0000) > .00001) resultat = resultat + (Double) ((mult) * (double) negativeSum / negativeQuantity);
              if (Math.abs(positiveQuantity-0.0000) > .00001) resultat = resultat + (Double) ((double) positiveSum / positiveQuantity);
